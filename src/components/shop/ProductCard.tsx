...
const ProductCard: React.FC<Props> = ({ product }) => {
  const {
    selectedProducts,
    selectProduct,
    removeSelectedProduct, // Assume this action exists in the store
  } = useStore();
  ...
  useEffect(() => {
    ...
    if (selectedProducts[product.itemId]) {
      setQuantity(selectedProducts[product.itemId].quantity as number);
  ...
  const handleQuantityChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value < 0) {
      ...
      // Optionally, prompt user to remove the product
      setQuantity(0);
      setError(
