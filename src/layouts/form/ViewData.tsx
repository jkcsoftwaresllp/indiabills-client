...
import { getData, getReport } from "../../network/api";
...
import { updateStuff, deleteStuff } from "../../network/api";
...
const ViewData = <T extends Services>({ title, url, initialColDefs, disableControls, menuOptions, dateRange }: Props<T>) => {
  ...
  const [colDefs, setColDefs] = useState<ColDef[]>(initialColDefs);
  ...
  initialSelectedColumns = initialSelectedColumns.filter((col: never) =>
  ...
  return (
    <PageAnimate>
        ...
        <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
          <ColumnSelector
            columns={colDefs as unknown as { field: string; headerName: string; editable: boolean }[]}
