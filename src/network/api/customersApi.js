...
export async function updateCustomerSelf(customerData) {
  try {
    ...
    if (customerData.loyalty_points !== undefined && customerData.loyalty_points < 0) {
      errors.push('Loyalty points must be non-negative');
    }
    if (customerData.is_active !== undefined && typeof customerData.is_active !== 'boolean') {
    ...
    const response = await serverInstance.patch('/internal/customers/self', customerData);
    return response.data;
  } catch (error) {
...
export async function createCustomerAddress(addressData) {
  try {
    ...
    if (addressData.pin_code && !/^[a-zA-Z0-9]{3,10}$/.test(addressData.pin_code)) {
      errors.push('Pin code must be 3-10 alphanumeric characters');
    }
    if (addressData.contact_phone && !/^\+?[0-9]{7,15}$/.test(addressData.contact_phone.replace(/\s/g, ''))) {
    errors.push('Contact phone must be 7-15 digits');
    }
    if (addressData.contact_email && !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(addressData.contact_email)) {
    errors.push('Invalid contact email format');
    }
    if (addressData.address_type && !['billing', 'shipping', 'other'].includes(addressData.address_type)) {
      errors.push('Address type must be billing, shipping, or other');
    }
    if (addressData.is_default !== undefined && typeof addressData.is_default !== 'boolean') {
    ...
    const response = await serverInstance.post('/internal/customers/address', addressData);
    return response.data;
  } catch (error) {
    console.error('Failed to create customer address:', error.response || error);
    throw error;
  }
}
...
export async function getCustomerAddress(id) {
  try {
    const response = await serverInstance.get(`/internal/customers/address/${id}`);
    return response.data;
  } catch (error) {
...
export async function updateCustomerAddress(id, addressData) {
  try {
    ...
    const response = await serverInstance.patch(`/internal/customers/address/${id}`, addressData);
    return response.data;
  } catch (error) {
    console.error(`Failed to update customer address ${id}:`, error.response || error);
    throw error;
  }
}
...
export async function deleteCustomerAddress(id) {
  try {
    const response = await serverInstance.delete(`/internal/customers/address/${id}`);
    return response.status;
  } catch (error) {
