...
import { Supplier } from "../../definitions/Types";
...
const colDefs: ColDef[] = [
  { field: "supplierId", headerName: "ID", width: 50, cellRenderer: (params: any) => (<p><span className="text-blue-950">#</span><span className="font-medium">{params.value}</span></p>) },
      ...
      onClick: (data?: Supplier,) => {
        deleteRow(`suppliers/soft/${data?.supplierId}`).then((response) => {
          if (response === 200) {
            ...
            refreshTableSetId(data?.supplierId as unknown as number);
            navigate("/suppliers");
          } else {
