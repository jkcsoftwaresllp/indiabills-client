...
const BatchItemsEditTab: React.FC<ItemsEditTabProps> = ({ items, availableItems, handleItemChange, handleDeleteItem, handleAddItem, }) => {

  const selectedItemIds = items.map((item) => item.itemId).filter((id) => id !== undefined && id !== null);
  const getAvailableItems = (currentItemId: number | undefined) => { return availableItems.filter( (item) => item.id === currentItemId || !selectedItemIds.includes(item.id) ); };
