...
const ViewInventory = () => {
  ...
  const colDefs = [
    ...
    { headerName: "Entry", field: "entryDate", width: 210, valueFormatter: ({ value }) => {
      if (!value) return '';
    ...
    { headerName: "Pass", field: "qualityPass", width: 80, cellRenderer: (params) => {
        switch(params.value) {
            case 'ok':
    ...
    { headerName: "Status", field: "status", width: 150, cellRenderer: (params) => (
          <span
  ...
  const fetchProducts = async (locationID) => {
    try {
      const response = await getBatches({ warehouseId: locationID });
      if (response.status === 200) {
        const sortedProducts = response.data.sort((a, b) => {
          const dateA = new Date(a.purchaseDate || a.entryDate).getTime();
          const dateB = new Date(b.purchaseDate || b.entryDate).getTime();
          return dateB - dateA;
        });
