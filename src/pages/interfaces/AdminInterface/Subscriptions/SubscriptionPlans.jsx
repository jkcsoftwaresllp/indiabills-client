...
import PartialAmountDialog from "./PartialAmountDialog";
...
const SubscriptionPlans = () => {
  ...
  const canRenewSubscription = () => {
    if (!currentSubscription || !currentSubscription.end_date) return true;
    const daysUntilExpiry = getDaysUntilExpiry(currentSubscription.end_date);
  ...
  const handleSelectPlan = (plan) => {
    ...
    // Show payment type dialog instead of cycle dialog
  ...
  const handlePartialAmountConfirm = async (amount) => {
    ...
    try {
      ...
      if (response.status === 200 && response.data.success) {
        ...
        setOrderError(
          response.data.message || "Failed to create partial payment order"
        );
      }
    } catch (err) {
      setOrderError("Error creating partial payment order");
      console.error(err);
    } finally {
  ...
  return (
    <Box sx={{ py: 1, px: 1 }}>
      {error && (
        <Alert
          severity="error"
          sx={{ mb: 2, borderRadius: 1.25, fontSize: "0.9rem" }}
        >
          {error}
        </Alert>
      )}
                ...
                sx={{
                  ...
                  boxShadow:
                    isFeatured || isCurrentPlan
                      ? "0 15px 30px rgba(30, 41, 56, 0.2)"
                      : "0 2px 8px rgba(0, 0, 0, 0.06)",
                  transition: "all 0.3s cubic-bezier(0.4, 0, 0.2, 1)",
                  borderRadius: 2,
                  overflow: "hidden",
                  opacity: shouldGreyOut ? 0.5 : 1,
                  pointerEvents: shouldGreyOut ? "none" : "auto",
                  "&:hover": {
                    boxShadow: shouldGreyOut
                      ? "0 2px 8px rgba(0, 0, 0, 0.06)"
                      : isFeatured || isCurrentPlan
                      ? "0 20px 40px rgba(30, 41, 56, 0.25)"
                      : "0 8px 16px rgba(0, 0, 0, 0.12)",
                    borderColor: shouldGreyOut ? "#e0e0e0" : "primary.main",
                  },
                }}
              >
                {isCurrentPlan ? (
                ...
                <CardContent
                  sx={{ flex: 1, pt: 2, px: 2.5, py: 2, overflowY: "auto" }}
                >
                  <Typography
                    variant="h6"
                    component="h2"
                  ...
                  <Box sx={{ mb: 1.5 }}>
                    ...
                    <List sx={{ py: 0 }}>
                      {plan.features && plan.features.length > 0 ? (
                        ...
                        <ListItem
                          sx={{ py: 0.45, px: 0, alignItems: "flex-start" }}
                        >
                          <CheckCircle
                            sx={{
                              color: "success.main",
                ...
                <CardActions sx={{ pt: 1, pb: 2, px: 2.5 }}>
                  ...
                  >
                    <Button
                      ...
                      sx={{
                        ...
                        "&:hover:not(:disabled)": {
                          boxShadow:
                            isFeatured && !isCurrentPlan
                              ? "0 8px 16px rgba(30, 41, 56, 0.2)"
                              : "0 4px 8px rgba(30, 41, 56, 0.15)",
                        },
                      }}
                    >
                      {isCurrentPlan
                    ...
                    {shouldGreyOut && (
                      ...
                      >
                        Other plans disabled until current plan expires
                      </Typography>
                    )}

                    {isCurrentPlan && currentSubscription?.end_date && (
                      <Typography
                        variant="caption"
                        sx={{
                          textAlign: "center",
