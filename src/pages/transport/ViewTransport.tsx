...
import {Supplier, Transport} from "../../definitions/Types";
import {deleteRow} from "../../network/api";

const colDefs: ColDef[] = [
  { field: "transportId", headerName: "ID", width: 50, cellRenderer: (params: any) => (<p><span className="text-blue-950">#</span><span className="font-medium">{params.value}</span></p>) },
      ...
      onClick: (data?: Transport,) => {
        deleteRow(`transport/delete/${data?.transportId}`).then((response) => {
              if (response === 200) {
                ...
                refreshTableSetId(data?.transportId as unknown as number);
                navigate("/transport");
              } else {
