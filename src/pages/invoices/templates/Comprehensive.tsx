...
import { Organization, InvoiceInfo } from "../../../definitions/Types";
import numberToWords from 'number-to-words';
import { formatDateDDMMYYYY } from "../../../utils/FormHelper";

const ComprehensiveReportTemplate: React.FC<{
  invoice: InvoiceInfo;
  initials: string;
  organization: Organization;
}> = ({ invoice, organization, initials }) => {

  const upiUrl = `upi://pay?pa=${organization.upi}&pn=${encodeURIComponent(organization.organizationName)}&cu=INR`;

  const formatPercent = (no: number | string) => {
    const num = Number(no);
  ...
  // Pre-calculate rate and total for each item
  ...
  // Calculate subtotal as sum of rate * quantity
  ...
  // Calculate totals for Rate, Tax, and Net Total columns
  const totals = {
    rate: parseFloat(
      processedItems
  ...
  return (
    <div id="invoice-container" className="w-full flex flex-col items-center justify-center text-xs">
      <div id="invoice-content" className="bg-white relative flex flex-col shadow-md rounded-xl py-3 px-4 min-w-full max-w-4xl">
        ...
        {/* Organization and Invoice Information */}
        ...
        <section className="p-2 flex justify-between w-full text-xs border-b-2">
          {/* Bank Details & Remarks Section */}
          <div className="flex flex-col w-full items-start text-nowrap"> {/* add `gap-1` here later */}
