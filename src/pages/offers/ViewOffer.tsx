...
import { Offer } from "../../definitions/Types";
...
const colDefs: ColDef[] = [
  { field: "offerId", headerName: "ID", width: 50, cellRenderer: (params: any) => (<p><span className="text-blue-950">#</span><span className="font-medium">{params.value}</span></p>) },
      ...
      onClick: (data?: Offer,) => {
        deleteRow(`offers/delete/${data?.offerId}`).then((response) => {
          if (response === 200) {
            ...
            refreshTableSetId(data?.offerId as unknown as number);
            navigate("/offers");
          } else {
