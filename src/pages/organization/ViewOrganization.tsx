...
const ViewOrganization = () => {
  ...
  useEffect(() => {
    const fetchOrganization = async () => {
      ...
      setOrganization(response as unknown as never);
    };
  ...
  return (
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-md rounded-lg">
      <div className="flex flex-col items-center">
        {organization.logo && (
          <img src={`${getBaseURL()}/${organization.logo}`} alt={`${organization.organizationName} logo`} className="w-32 h-32 mb-4" />
        )}
      ...
      <div className="flex flex-col items-center">
        <div className="w-full">
          <p className="mb-4"><strong>Type:</strong> {organization.organizationType}</p>
          <p className="mb-4"><strong>Industry:</strong> {organization.industry}</p>
          <p className="mb-4"><strong>Phone:</strong> {organization.phone}</p>
          <p className="mb-4"><strong>Email:</strong> {organization.email}</p>
          <p className="mb-4"><strong>Website:</strong> <a href={organization.website} target="_blank" rel="noopener noreferrer" className="text-blue-500">{organization.website}</a></p>
          <p className="mb-4"><strong>Address:</strong> {organization.addressLine}</p>
        </div>
      </div>
    </div>
    </PageAnimate>
