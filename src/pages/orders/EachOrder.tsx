...
interface Props {
  ...
  handleMarkPayment: (orderId: number, status: string) => void;
  handleChangeOrderStatus: (orderId: number, status: string) => void;
}

export const EachOrder: FC<Props> = ({
  order,
  timelineCollapsed,
  toggleTimeline,
  handleMarkPayment,
  ...
  const containerRef = useRef<HTMLDivElement>(null); // Attach a ref to the container
  ...
  const [isItemsVisible, setIsItemsVisible] = useState(false); // State for items visibility
  ...
  const handleContextMenu = (event: React.MouseEvent) => {
    ...
    const getAvailableStatusSubItems = (currentStatus: string) => {
          ...
          return [
            {
              label: "Shipped",
              onClick: () =>
                handleChangeOrderStatus(order.orderId, "shipped"),
            },
          ];
        case "fulfilled":
          return [
            {
              label: "Returned",
              onClick: () =>
                handleChangeOrderStatus(order.orderId, "returned"),
            },
          ];
        case "cancelled":
          return [
    ...
    >
      <AnimatePresence>
          ...
          >
            <section id="OrderHead" className="z-40">
              ...
              <div className="flex items-center justify-between mb-1">
                <h1 className="text-black text-md font-light">
      ...
      <h3
          ...
          >{order.items.length} Items
              ...
              <ItemSection order={order as never} toggleItemsVisibility={toggleItemsVisibility} isItemsVisible={isItemsVisible} />
            </section>
