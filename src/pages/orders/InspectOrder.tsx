...
import { Customer, Item } from "../../definitions/Types";
...
const InspectOrder = () => {
  ...
  const handleDeleteItem = (id: string) => {
    setFormState((prevState: any) => ({
      ...prevState,
      items: prevState.items.filter((item: any) => item.id !== id),
    }));
  };
  ...
  const [products, setProducts] = useState<Partial<Item>[]>(
    [] as Partial<Item>[],
  );
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      ...
      // console.log(pdata);
      setProducts(pdata as Partial<Item>[]);
      const cdata = await getStuff("customers/options/list");
      setCustomers(cdata as CustomerOptions[]);
    };
    ...
    <PageAnimate>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <ItemsEditTable
          ...
          availableItems={products as unknown as never}
